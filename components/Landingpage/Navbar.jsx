...
export default function Navbar() {
  ...
  useEffect(() => {
    const checkScreen = () => setIsDesktop(window.innerWidth >= 300);
    checkScreen();
    window.addEventListener("resize", checkScreen);
    return () => window.removeEventListener("resize", checkScreen);
  }, []);

  const whiteBgPaths = ["/about", "/contact", "/products", "/our-blogs"];
  const isWhiteBg =
...
{mobileOpen && (
  <nav className="md:hidden bg-white/95 backdrop-blur-md shadow-md px-6 py-4 space-y-4 text-[#0075C9] font-semibold">
    <Link href="/" className="block w-full py-2 border-b border-gray-200">
      Home
    </Link>
    <Link href="/about" className="block w-full py-2 border-b border-gray-200">
      About Us
    </Link>

    <details className="group border-b border-gray-200">
      <summary className="cursor-pointer flex justify-between items-center py-2">
        Our Products <ChevronDown size={16} className="group-open:rotate-180 transition-transform" />
      </summary>
    ...
    <Link href="/our-blogs" className="block w-full py-2 border-b border-gray-200">
      Our Blogs
    </Link>
    <Link href="/contact" className="block w-full py-2 border-b border-gray-200">
      Contact Us
    </Link>
